...
<tr><td align="center" style="padding-top:28px"><p style="margin:0;font-size:12px;color:#555873;line-height:1.5">HireAxis — AI-powered interviews for modern hiring teams<br><a href="https://hireaxis.ai" style="color:#5b6cf7;text-decoration:none">hireaxis.ai</a></p></td></tr>
...
export async function sendVerificationEmail(email, name, code) {
  return sendEmail({
    ...
    html: baseTemplate(`
      ...
      <p style="margin:0 0 28px;font-size:15px;color:#7a7d9a;line-height:1.6">Hi ${name}, welcome to HireAxis! Enter this code to verify your email and activate your account.</p>
  ...
  return sendEmail({
    ...
    html: baseTemplate(`
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px">
        ${[['1','Create a role','Add a job description — AI generates interview questions'],['2','Share your interview line','Candidates call or interview via browser'],['3','Review scorecards','AI scores every interview with detailed feedback']].map(([n,t,d])=>`<tr><td style="padding:12px 0"><table cellpadding="0" cellspacing="0"><tr><td style="width:32px;height:32px;border-radius:8px;background:rgba(91,108,247,0.12);text-align:center;vertical-align:middle;font-size:13px;font-weight:700;color:#5b6cf7">${n}</td><td style="padding-left:14px"><div style="font-size:14px;font-weight:600;color:#eeeef5;margin-bottom:2px">${t}</div><div style="font-size:13px;color:#7a7d9a">${d}</div></td></tr></table></td></tr>`).join('')}
      </table>
      ...
      <div style="background:#06070a;border:1px solid #1a1c2e;border-radius:10px;padding:16px 20px;text-align:center"><div style="font-size:12px;color:#555873">Your trial: <strong style="color:#7a7d9a">14 days</strong> · <strong style="color:#7a7d9a">20 interviews</strong> · <strong style="color:#7a7d9a">No credit card</strong></div></div>
    `, `Your HireAxis account is ready. Start interviewing candidates with AI.`)
  });
}
...
export async function sendScorecardEmail(email, candidateName, roleTitle, score, recommendation) {
  ...
  return sendEmail({
    ...
    html: baseTemplate(`
      <h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#eeeef5">New interview scorecard</h1>
      ...
      <div style="background:#06070a;border:1px solid #1a1c2e;border-radius:12px;padding:24px;text-align:center;margin-bottom:24px">
        <div style="font-size:48px;font-weight:800;color:${scoreColor};font-family:'Courier New',monospace;margin-bottom:4px">${score}</div>
        <div style="font-size:12px;color:#555873;margin-bottom:12px">Overall score</div>
      ...
      <div style="text-align:center"><a href="https://hireaxis-dashboard.vercel.app" style="display:inline-block;padding:12px 28px;background:#5b6cf7;color:#fff;text-decoration:none;border-radius:10px;font-size:14px;font-weight:600">View full scorecard</a></div>
    `, `${candidateName} scored ${score}/100. Recommendation: ${recLabel}`)
  });
}

export default { sendVerificationEmail, sendWelcomeEmail, sendScorecardEmail };